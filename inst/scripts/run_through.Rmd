...
example_segs_data <- update_segments_data(con = con, ais_data = example_ais_data)
...
write_new_segs_data <- function(data=NULL, con=NULL){
    if (is.null(data)==TRUE){
     print("Null segments data")
    } else {
      dbWriteTable(con = con, 
                   name = 'ais_segments_temp', 
                   value = data, 
                   overwrite = TRUE)
    }
  }
...
# Run it
write_new_segs_data(data = example_segs_data, con = con)
```
...
```{r, add updated vsr_segments_temp data into vsr_segments table}

dbExecute(con, "INSERT INTO vsr_segments_test
...
```{r, vsr_ex_map}
con <- db_connect()
ais_segments_ex <- sf::st_read(dsn = con, EWKB = TRUE, 
