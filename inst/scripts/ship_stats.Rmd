...
png("/Users/seangoral/Library/Mobile Documents/com~apple~CloudDocs/_Benioff_SRA_Limited/100kHighRes150dpi2.png", units="px", width=800, height=800, res=150)
theme_set(theme_bw()) 
ggplot(data.frame(ship_stats_2019_300_100k), aes(x=grade)) +
  geom_bar() +  
  theme(plot.title = element_text(hjust = 0.5)) + 
  ggtitle("Ship VSR Cooperation Grade Distribution: 2019") + 
  labs(caption="Source: Benioff Ocean Initiative",
       x="VSR Cooperation Grade",
       y="Count") +
   geom_text(stat='count', aes(label=..count..), vjust=1.5, color="white")+
...
ggplot(sum_stats, aes(x="", y=value, fill=variable)) + geom_bar(stat="identity", width=1) + 
  coord_polar("y", start=0) + geom_text(aes(label = paste0(round(value), "%")), position = position_stack(vjust = 0.5)) + 
...
```{r}
png("/Users/seangoral/Library/Mobile Documents/com~apple~CloudDocs/_Benioff_SRA_Limited/op_count.png", units="px", width=800, height=800, res=150) 
ggplot(data.frame(operator_stats_2019_300_100k), aes(x=operator_stats_2019_300_100k$number_of_distinct_mmsi)) +
  geom_bar() +  
  theme(plot.title = element_text(hjust = 0.5)) + 
  ggtitle("MMSI's Per Operator: 2019") + 
  labs(caption="Source: Benioff Ocean Initiative",
       x="Vessel (MMSI) Count",
       y="Operator Count") +
