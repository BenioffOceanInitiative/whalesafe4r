...
g = ggplot(data.frame(operator_stats_2019_300_100k), aes(x=grade)) +
  geom_bar() +  
...
length(unique(vsr_segs_ihs_filtered$operator))
  ...
  ship_stats_filtered = vsr_segs_ihs_filtered[, list(
    #datetime = beg_dt,
  ...
  ship_stats_filtered$grade = cut(ship_stats_filtered$`compliance score (reported speed)`,
                      breaks = c(0, 60, 70, 80, 90, 99, 100),
                      labels = c("F", "D", "C", "B", "A", "A+"),
                      right = FALSE,
                      include.lowest = TRUE)
  # order ship_stats data.table ----
  ship_stats_filtered <- ship_stats_filtered[order(-grade, -`total distance (nautcal miles)`)]
...
summary_stats = data.table(  
  under_10_nm=sum(ship_stats_filtered$`distance (nautcal miles) under 10 knots`),
...
summary_stats_percents = data.table(  
  under_10_perc=(sum(ship_stats_filtered$`distance (nautcal miles) under 10 knots`)/sum(ship_stats_filtered$`total distance (nautcal miles)`)),
  nm_10_12_perc=sum(ship_stats_filtered$`distance (nautcal miles) 10-12 knots`)/sum(ship_stats_filtered$`total distance (nautcal miles)`),
  nm_12_15_perc=sum(ship_stats_filtered$`distance (nautcal miles) 12-15 knots`)/sum(ship_stats_filtered$`total distance (nautcal miles)`),
  nm_over_15_perc=sum(ship_stats_filtered$`distance (nautcal miles) over 15 knots`)/sum(ship_stats_filtered$`total distance (nautcal miles)`))
